...
from .mcff import ChannelwiseFusion
...
class ResnetEncoderMatching(nn.Module):
    ...
    def feature_extraction(self, image,masknet_featlst0, return_all_feats=False):
        ...
        feats_1 = self.MCFF(feats_1, masknet_featlst0, 24, 80, 1)
    ...
    def forward(self, current_image, lookup_images, poses, K, invK,masknet_featlst,
                min_depth_bin=None, max_depth_bin=None
                ):

        # feature extraction
        self.features = self.feature_extraction(current_image,masknet_featlst[0], return_all_feats=True)
        ...
        with torch.no_grad():
            ...
            lookup_feats = self.feature_extraction(lookup_images,masknet_featlst[0],
        ...
        MCFF2 = self.MCFF(self.features[-1], masknet_featlst[1], 12, 40, 2)

        self.features.append(self.layer3(MCFF2))

        MCFF3 = self.MCFF(self.features[-1], masknet_featlst[2], 6, 20, 3)

        tem = self.layer4(MCFF3)

        self.features.append(self.MCFF(tem, masknet_featlst[3], 6, 20, 4))
...
class PoseResnetEncoder(nn.Module):
...
class ResnetEncoder(nn.Module):
    ...
    def forward(self, input_image,masknet_featlst):
        self.features = []
        x = (input_image - 0.45) / 0.225
        x = self.encoder.conv1(x)
        x = self.encoder.bn1(x)
        ...
        MCFF1 = self.MCFF(self.features[-1], masknet_featlst[0], 24, 80, 1)

        self.features.append(self.encoder.layer2(MCFF1))

        MCFF2 = self.MCFF(self.features[-1], masknet_featlst[1], 12, 40, 2)

        self.features.append(self.encoder.layer3(MCFF2))

        MCFF3 = self.MCFF(self.features[-1], masknet_featlst[2], 6, 20, 3)

        tem = self.encoder.layer4(MCFF3)

        self.features.append(self.MCFF(tem, masknet_featlst[3], 6, 20, 4))

        del tem,MCFF1,MCFF2,MCFF3
